...
- [**cstr/Octen-Embedding-0.6B-ONNX**](https://huggingface.co/cstr/Octen-Embedding-0.6B-ONNX) — 1024d, decoder, last-token pooling. Shipped as FP32 (`OctenEmbedding0_6BFp32`), INT4 MatMulNBits (`OctenEmbedding0_6BInt4`), and INT4-Full (~434 MB, `OctenEmbedding0_6BInt4Full`, INT4 MatMul + INT8 embedding table). The INT8-Full variant (`OctenEmbedding0_6BInt8Full`) was dropped after validation: it shows the same activation-outlier problem as F2LLM INT8 (cos≈0.60 vs PyTorch reference). The remaining INT4 variants pass at cos≈0.92.
- [**cstr/F2LLM-v2-0.6B-ONNX**](https://huggingface.co/cstr/F2LLM-v2-0.6B-ONNX) — 1024d, 200+ languages, Qwen3 decoder, last-token pooling. Shipped as FP32 (`F2LlmV2_0_6BFp32`) and INT4 MatMulNBits (`F2LlmV2_0_6BInt4`, [cstr/F2LLM-v2-0.6B-ONNX-INT4](https://huggingface.co/cstr/F2LLM-v2-0.6B-ONNX-INT4)). The INT8 / INT8-Full variants were dropped after validation against the PyTorch reference: dynamic INT8 quantization of the MatMul weights cannot represent the activation outliers typical of decoder LLMs, so per-sentence cosine collapses to ≈0.20–0.30. The INT4 variant recovers more of the quality (cos≈0.64) but is still reduced compared to FP32 — treat as memory-optimized rather than reference-quality.
...
- [**onnx-community/harrier-oss-v1-270m-ONNX**](https://huggingface.co/onnx-community/harrier-oss-v1-270m-ONNX) — 640d, 94 languages, Gemma3-text decoder, last-token pooling (`HarrierOSSV1_270M`); quantized INT8 variant also available (`HarrierOSSV1_270MQ`, ~344 MB). The quantized file uses ONNX Runtime's `GatherBlockQuantized` with the `bits` attribute, which requires ORT >= 1.23 — supported by `ort = "2.0.0-rc.12"`. Validation cosine vs PyTorch reference: `HarrierOSSV1_270MQ` 0.99993.
