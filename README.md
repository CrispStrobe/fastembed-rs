...
- [**zeroentropy/zerank-1-small**](https://huggingface.co/zeroentropy/zerank-1-small) — 1.7B Qwen3-based multilingual reranker (`ZerankSmall` FP32 / `ZerankSmallInt8` / `ZerankSmallInt4`). Quality is acceptable (Int8 Spearman 0.965 vs FP32 ONNX, Int4 Spearman 0.935; both diverge slightly from FP32 on multilingual ranking). However the underlying ONNX export at [cstr/zerank-1-small-ONNX](https://huggingface.co/cstr/zerank-1-small-ONNX) has a **batch=1 hardcode in the attention-mask broadcast** that affects all three variants — calling them with batch > 1 raises `Shape mismatch {1,1,T,T} != {B,1,T,T}` from ORT. Use single-pair inference, or wait for a re-export with proper dynamic batch axes.
