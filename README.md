...
- The **`zeroentropy/zerank-1-small`** variants (FP32 / INT8 / INT4) were removed.  The cstr ONNX exports have a hardcoded batch=1 in the attention-mask broadcast (the And kernel can't broadcast `{1,1,T,T}` against `{B,1,T,T}`), and fastembed-rs's reranker calls with `DEFAULT_BATCH_SIZE=256`, so any user call would raise `RuntimeException: Shape mismatch`.  Disabling ORT graph optimization doesn't help — the issue is in the exported graph itself.  Re-add once a re-export with proper dynamic batch axes is published.
