...
- [**cstr/Octen-Embedding-0.6B-ONNX**](https://huggingface.co/cstr/Octen-Embedding-0.6B-ONNX) — 1024d, decoder, last-token pooling. Shipped as FP32 (`OctenEmbedding0_6BFp32`), INT4 MatMulNBits (`OctenEmbedding0_6BInt4`, cos≈0.92), and INT4-Full (~434 MB, `OctenEmbedding0_6BInt4Full`, INT4 MatMul + INT8 embedding table, cos≈0.92). The INT8-Full variant was dropped (cos≈0.60: same activation-outlier problem as F2LLM INT8). FP16 (cos=1.000, 1.2 GB, streaming converter) is verified and pending HF re-upload.
- [**cstr/F2LLM-v2-0.6B-ONNX**](https://huggingface.co/cstr/F2LLM-v2-0.6B-ONNX) — 1024d, 200+ languages, Qwen3 decoder, last-token pooling. Shipped as FP32 (`F2LlmV2_0_6BFp32`) and INT4 MatMulNBits (`F2LlmV2_0_6BInt4`, [cstr/F2LLM-v2-0.6B-ONNX-INT4](https://huggingface.co/cstr/F2LLM-v2-0.6B-ONNX-INT4)). The INT8 / INT8-Full variants were dropped after validation against the PyTorch reference because the standard `quantize_dynamic(MatMul, QInt8)` recipe collapses cosine to ≈0.20–0.30 due to activation outliers typical of decoder LLMs; the INT4 variant recovers cos≈0.64 (top-1 retrieval preserved, per-sentence drift). **Re-quantization is feasible:** SmoothQuant (Xiao 2023, alpha=0.8) plus per-channel dynamic INT8 has been verified locally to reach cos_min≈0.93 vs the PyTorch reference, and a streaming FP32 → FP16 path has been verified at cos_min≈1.000 (50% memory savings). Both are pending HF re-upload before being re-added as `F2LlmV2_0_6BInt8` and `F2LlmV2_0_6BFp16`.
